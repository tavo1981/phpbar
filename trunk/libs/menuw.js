...
function menuItemMouseover(event, menuId) {
  ...
  function menuItemKeyDown(event, menuId) {
  active= null;
  ...
  // Initialize the sub menu, if not already done.
    ...
    if (item.subMenu.isInitialized == null)
       menuInit(item.subMenu);
       tags = item.subMenu.childNodes;
       a = 0;
       
       for (ji = 0;ji < tags.length;ji++){
         if (tags[ji].nodeName =='A'){
            if(event.which == 39){
              tags[ji].focus();
              item.id='mnu_active';
              active = item.id;
...
function proximo(item,e){
  ...
  function moveFirst(menu,event){
     tags = document.getElementById(menu).childNodes;
