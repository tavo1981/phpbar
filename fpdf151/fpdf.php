...
{   
    $this->SetY(-10);
    $this->AliasNbPages();
    $this->line(5,$this->h-10,$this->w-5,$this->h-10);
    $this->SetFont('Arial','B',8);   
    $this->Cell(50,3,$this->page.'/{nb}');
}
function PageNo()
{
...
//retangulos com borda arredondada
...
function WriteHTML($html)
    ...
    function cabecalho ($fonte="times"){
        if ($this->DefOrientation == "P"){
            $this->Image('libs/imagens/Brasao.png',10,10,20,28);
            $this->Image('libs/imagens/logocab.jpg',175,10,30,28);
        }elseif ($this->DefOrientation == "L"){
            $this->Image('libs/imagens/Brasao.png',10,10,20,28);
            $this->Image('libs/imagens/logocab.jpg',250,10,30,28);
        }
        $this->sety(15);
        $this->setfont($fonte,'b',12);
        $this->cell(0,5,"SECRETARIA MUNICIPAL DE EDUCAÇÃO E DESPORTO",0,1,"C");
        $this->setfont($fonte,'b',11);
        $this->cell(0,5,"PREFEITURA MUNICIPAL DE SAPIRANGA",0,1,"C");
        $this->setfont($fonte,'b',10);
        $this->cell(0,5,"ESTADO DO RIO GRANDE DO SUL",0,1,"C");
        $this->setfont($fonte,'b',12);   
          

    }
    function cabecalho_esc($titulo="",$headers="",$fonte="times"){
        $this->SetFont($fonte,"",12);
        $this->setxy(5,5);
        $this->cell(50,5,"Prefeitura Municipal de Sapiranga",0,1);   
        $this->setx(5);
        $this->SetFont($fonte,"",10);
        $this->cell(50,5,$_SESSION["usu_inst"],0,1);
        $this->Text(($this->w - 50),10,"Emissão: ".date("d/m/Y"));
        $y = 15;
       // print_r($headers);
       if (is_array($headers)){
            
            for ($i = 0;$i < count($headers);$i++){
                 $this->Text(($this->w - 50),$y,$headers[$i]);
                 $y += 5;
            }
        }
        //$this->sety($y);
...
//por ela tbm pode se definir textto em negrito, fonte...
